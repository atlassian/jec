	...
	for {
		...
		case <-time.After(interval):
			if _, err := os.Stat(logger.Filename); os.IsNotExist(err) {
				logrus.Warnf("Failed to open JEC log file: %v. New file will be created.", err)
				if err = logger.Rotate(); err != nil {
					logrus.Warn(err)
				} else {
					logrus.Warnf("New JEC log file[%s] is created, previous one might be removed accidentally.", logger.Filename)
				}
				break
			}
		}
