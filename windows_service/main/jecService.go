...
func (p *Program) Start(s service.Service) error {
	p.cmd = exec.Command("cmd", append([]string{"/C", p.JECPath}, p.Args...)...)
...
func (p *Program) run() {
	...
	err := p.cmd.Run()
	if err != nil {
		logger.Errorf("Failed to run JEC: %v", err)
	}
...
func validateConfig(config *Config) {
	...
	_, err := os.Stat(config.JECPath)
	if err != nil {
		log.Fatalf("Failed to find JEC executable %q: %v", config.JECPath, err)
	}
}
...
func main() {
	...
	if len(os.Args) > 1 {
		...
		if action == "stop" {
			log.Println("JEC service stopped successfully.")
		} else {
			log.Println("JEC service " + action + "ed successfully.")
		}
